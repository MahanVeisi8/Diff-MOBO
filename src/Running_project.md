...
-   The `init_cl_i_j_path`: for setting up each pipeline weigths for the surrogate_model(which is ensemble) 
...
First we have to change this code to only run the `python3 innerloop_creation_3.py --iteration "$i"` on Docker and CPU(no need for GPU!) and rest on GPU cluster.
...
Also in the `src/OpenFoam/` we added one file, `src/OpenFoam/innerloop_performance_finding.py` which is responsible for running the airfoil shapes specificly for the innerloop codes and is super   important to set the `NUM_CORE` in there.
...
-   `init_cl_i_j_path`: same as the stage1.


after running this we should have `src/optimization_loop/Inner_loop/UA_surrogate_weights/updated/UA_surrogate_model_8_channel.pt` for UA_weigths and the `DB_valids` and `DB_invvalids`
