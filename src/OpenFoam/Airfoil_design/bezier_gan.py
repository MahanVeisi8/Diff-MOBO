...
#        S = 1/(1+D)
    ...
    def train(self, X_train, train_steps=2000, batch_size=32, disc_lr=2e-4, gen_lr=2e-4, save_interval=0,
        ...
        graph = tf.get_default_graph()
        tf.graph_util.import_graph_def(frozen_graph,
                                       input_map={'x:0': self.x_fake_test, 'training:0': False},
                                       name='surrogate')
        self.y_test = graph.get_tensor_by_name('surrogate/net/y:0')
        tf.identity(self.y_test, name='out_y')
        #calculate the jacobian with respect to the input
        grad_out_c_0 = tf.gradients(self.y_test[:, 0], [self.c])
        # grad_out_z_0 = tf.gradients(self.y_test[:, 0], [self.z])
        grad_out_c_1 = tf.gradients(self.y_test[:, 1], [self.c])
        # grad_out_z_1 = tf.gradients(self.y_test[:, 1], [self.z])
        # grad_out_0 = tf.concat([grad_out_c_0[0], grad_out_z_0[0]], axis=-1)
        # grad_out_1 = tf.concat([grad_out_c_1[0], grad_out_z_1[0]], axis=-1)
        # self.jac_out = tf.concat([grad_out_0, grad_out_1], axis=0)
        self.jac_out = tf.concat([grad_out_c_0[0], grad_out_c_1[0]], axis=0)
        tf.identity(self.jac_out, name='out_jac')

        #calculate the hession with respect to the input
        var_hess_0 = tf.hessians(self.y_test[:, 0], [self.c])[0][0,:,0,:]
        var_hess_1 = tf.hessians(self.y_test[:, 1], [self.c])[0][0,:,0,:]
        self.hess_out = tf.stack([var_hess_0, var_hess_1], 0)
        tf.identity(self.hess_out, name='out_hess')

        # var_grad_all = []
        # for i in range(5):
        #     var_grad_c_c = tf.gradients(tf.gradients(self.y_test[:, 0], [self.c])[0][:, i], [self.c])[0]
        #     var_grad_c_z = tf.gradients(tf.gradients(self.y_test[:, 0], [self.c])[0][:, i], [self.z])[0]
        #     var_grad_c_ = tf.concat([var_grad_c_c, var_grad_c_z], -1)
        #     var_grad_all.append(var_grad_c_[0])
        # for i in range(10):
        #     var_grad_z_c = tf.gradients(tf.gradients(self.y_test[:, 0], [self.z])[0][:, i], [self.c])[0]
        #     var_grad_z_z = tf.gradients(tf.gradients(self.y_test[:, 0], [self.z])[0][:, i], [self.z])[0]
        #     var_grad_z_ = tf.concat([var_grad_z_c, var_grad_z_z], -1)
        #     var_grad_all.append(var_grad_z_[0])
        # var_grad_0 = tf.stack(var_grad_all)

        # var_grad_all = []
        # for i in range(5):
        #     var_grad_c_c = tf.gradients(tf.gradients(self.y_test[:, 1], [self.c])[0][:, i], [self.c])[0]
        #     var_grad_c_z = tf.gradients(tf.gradients(self.y_test[:, 1], [self.c])[0][:, i], [self.z])[0]
        #     var_grad_c_ = tf.concat([var_grad_c_c, var_grad_c_z], -1)
        #     var_grad_all.append(var_grad_c_[0])
        # for i in range(10):
        #     var_grad_z_c = tf.gradients(tf.gradients(self.y_test[:, 1], [self.z])[0][:, i], [self.c])[0]
        #     var_grad_z_z = tf.gradients(tf.gradients(self.y_test[:, 1], [self.z])[0][:, i], [self.z])[0]
        #     var_grad_z_ = tf.concat([var_grad_z_c, var_grad_z_z], -1)
        #     var_grad_all.append(var_grad_z_[0])
        # var_grad_1 = tf.stack(var_grad_all)

        # self.hess_out = tf.stack([var_grad_0, var_grad_1], 0)
        # tf.identity(self.hess_out, name='out_hess')
...
#                          scale=.95, scatter=True, s=1, alpha=.7, fname='{}/synthesized'.format(directory))
    ...
    def restore(self, directory='.'):
        ...
        self.y_test = graph.get_tensor_by_name('out_y:0')
        self.jac_out = graph.get_tensor_by_name('out_jac:0')
        self.hess_out = graph.get_tensor_by_name('out_hess:0')
