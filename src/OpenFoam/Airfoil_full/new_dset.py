...
def airfoil_to_performance(n_iter, n_batch):
    ...
    correct_order_index = performance[:, 2].astype(int)
    latent_corect_order = np.array(latent)
    latent_corect_order = latent_corect_order[correct_order_index,:]
    sio.savemat(f'./Dataset/temp/dset_design_{n_batch}_{n_iter}.mat', {'latent': latent_corect_order})
