...
for n_iter in range(5):
    ...
    os.system('ssh slurm-submit.mpi-inf.mpg.de source /HPS/NavidCAM/work/Creative_GAN/MONBO_Automized/run_MONBO_slurm.sh')
    dir_path = 'Models/iter_%d' %(n_iter)
    
    
    # Train the constraint handler
    sh_file_path = 'train_constraint_handler.sh'
    new_line = "n_iter=%d" %(n_iter)
    #edit the bash file with the new iteration number
    with open(sh_file_path, "r") as sh_file:
        lines = sh_file.readlines()
    ...
    print('Training the constraint handler...')
    os.system('ssh slurm-submit.mpi-inf.mpg.de source /HPS/NavidCAM/work/Creative_GAN/MONBO_Automized/run_constraint_handler.sh')
    ...
    if not os.path.exists(new_dir):
        ...
        print("Directory already exists:", new_dir)
    os.system('ssh slurm-submit.mpi-inf.mpg.de source /HPS/NavidCAM/work/Creative_GAN/MONBO_Automized/run_acquisition_slurm.sh')
