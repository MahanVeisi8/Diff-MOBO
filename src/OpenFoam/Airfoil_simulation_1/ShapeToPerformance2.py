...
def calculate_cd_cl(n, num_cores, airfoil_sample_train , batch_num):
    ...
    os.makedirs(f"./Airfoil_simulation_1/results_{batch_num}", exist_ok=True)
    ...
    for sample_num in range(n * devision_factor, (n + 1) * devision_factor):
        ...
        if genMesh(airfoil_sample_train[sample_num, :, :]) != 0:
            ...
            CD = 1000
            os.chdir("../..")
            a_file = open(f"./Airfoil_simulation_1/results_{batch_num}/%d.txt" % (sample_num), "w")
...
def calculate_cd_cl_res(n, sample_num, airfoil_sample_train, batch_num):
    # For each core assign TotalSampls/NumberOfCores
    # print(devision_factor)
    # for sample_num in range(n * devision_factor, (n + 1) * devision_factor):
    print("core {}:".format(n))
    ...
    os.makedirs(f"./Airfoil_simulation_1/results_{batch_num}/", exist_ok=True)
    if not os.path.exists(f"./Airfoil_simulation_1/results_{batch_num}/%d.txt" % (sample_num)):
        with open(f"./Airfoil_simulation_1/results_{batch_num}/%d.txt" % (sample_num), "w") as f:
            pass
...
def shape_to_performance(airfoil_sample_train, airfoil_latent , batch_num):
    jobs = []
    ...
    if airfoil_sample_train.shape[0] > num_cores:
        for n in range(0, num_cores):
            p = multiprocessing.Process(target=calculate_cd_cl, args=(n, num_cores, airfoil_sample_train , batch_num))
            jobs.append(p)
            p.start()
            processes.append(p)
    for p in processes:
    ...
    if devision_factor*num_cores < airfoil_sample_train.shape[0]:
        for n in range(0, airfoil_sample_train[devision_factor*num_cores:,:,:].shape[0]):
            p = multiprocessing.Process(target=calculate_cd_cl_res, args=(n, devision_factor*num_cores + n, airfoil_sample_train, batch_num))
            jobs.append(p)
            p.start()
            processes.append(p)
    ...
    Data_path = f"./Airfoil_simulation_1/results_{batch_num}/"
