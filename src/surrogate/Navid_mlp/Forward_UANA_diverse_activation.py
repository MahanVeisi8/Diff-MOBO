...
for dset in range(iter_n+1):
    mat = sio.loadmat('Dataset/dset_%d.mat' %(dset))
    mat = mat['dset']
    Design_data = mat['X'][0][0]
    Performance_data = mat['Y'][0][0]
    if dset == 0:  # Initialize arrays in the first iteration
        ...
        Performance_data_all = np.concatenate((Performance_data_all, Performance_data), axis=0)



Design_data = Design_data_all
Performance_data = Performance_data_all[:,:2]

Performance_data[:,1]=Performance_data[:,0]/Performance_data[:,1]
min_y = np.min(Performance_data, axis=0, keepdims=True)
print(min_y)
max_y = np.max(Performance_data, axis=0, keepdims=True)
print(max_y)
Performance_data = (Performance_data-min_y)/(max_y-min_y)

print(Performance_data.shape)

x_train_tensor = torch.from_numpy(Design_data).float()
y_train_tensor = torch.from_numpy(Performance_data).float()
